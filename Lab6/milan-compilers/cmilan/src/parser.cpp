...
void Parser::statement()
{
	...
	if(match(T_IF)) {
		...
		mustBe(T_LPAREN);//”(”
		expression();
		int switchAddr = lastVar_++;
		//загружаем в память результат выражения и записываем его опять в стек
		codegen_->emit(STORE, switchAddr);
		codegen_->emit(LOAD, switchAddr);
		mustBe(T_RPAREN);//”)”
